...
import { useGetPlanningSeriesQuery } from "../api/plannigSeriesApi";
import type {
    PlanningSeries,
    FiltersRecord,
    PlanningSeriesResponse,
} from "../model/planningSeriesType";
import { tableHead } from "../model/planningSeriesIndex";

function getValueByPath(obj: PlanningSeries, path: string): unknown {
    if (!obj || !path) return "";
...
export const usePlanningSeriesData = () => {
    ...
    const { data: planningSeries, isLoading } = useGetPlanningSeriesQuery(
        paramsfilterData,
        {
            refetchOnMountOrArgChange: true,
        }
    );
    const [displayData, setDisplayData] = useState<PlanningSeriesResponse | null>(null);

    useEffect(() => {
        if (planningSeries) {
            setDisplayData(planningSeries);
        }
    }, [planningSeries]);
    ...
    const handleSearch = (searchTerm: string) => {
        ...
        if (planningSeries) {
            const result = planningSeries.data.filter((row) =>

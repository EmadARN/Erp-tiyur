...
import { useGetPoultryCuttingExportsQuery } from "../api/poultryCuttingExportApi";
import type {
    PoultryCuttingExport,
    FiltersRecord,
    PoultryCuttingExportsResponse,
} from "../model/poultryCuttingExportType";
import { tableHead } from "../model/poultryCuttingExportIndex";

function getValueByPath(obj: PoultryCuttingExport, path: string): unknown {
    if (!obj || !path) return "";
...
export const usePoultryCuttingExportData = () => {
    ...
    const { data: poultryCuttingExports, isLoading } = useGetPoultryCuttingExportsQuery(
        paramsfilterData,
        {
            refetchOnMountOrArgChange: true,
        }
    );
    const [displayData, setDisplayData] = useState<PoultryCuttingExportsResponse | null>(null);

    useEffect(() => {
        if (poultryCuttingExports) {
            setDisplayData(poultryCuttingExports);
        }
    ...
    const handleSearch = (searchTerm: string) => {
        if (searchTerm === "" && poultryCuttingExports) {
            setDisplayData(poultryCuttingExports);
            return;
        }
        ...
        if (poultryCuttingExports) {
            const result = poultryCuttingExports.data.filter((row) =>
    ...
    return {
        poultryCuttingExports: displayData,
        isLoading,
        filterData,
        setFilterData,

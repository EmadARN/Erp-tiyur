...
} from "../api/productionSeriesApi";
import { handleApiError } from "@/modules/shared/lib/handleApiError";
import type { CreateProductionSeriesDto } from "../model/productionSeriesType";
import { updateDialogDocs } from "../model/productionSeriesIndex";
...
export const useProductionSeriesActions = () => {
    const navigate = useNavigate();
    const [postProductionSeries] = usePostProductionSeriesMutation();
    const [patchProductionSeries] = usePatchProductionSeriesMutation();
    const [deleteProductionSeries] = useDeleteProductionSeriesMutation();
    const [deleteBulkProductionSeries] = useDeleteBulkProductionSeriesMutation();

    const deleteHandler = async (id: string) => {
        try {
            await deleteProductionSeries({ id }).unwrap();
    ...
    const bulkDeleteHandler = async (arrayIndex: string[]) => {
        try {
            await deleteBulkProductionSeries({
    ...
    const handleCreateConfirm = async (data: Record<string, any>) => {
        const formattedData: Partial<CreateProductionSeriesDto> = {
            product_owner: data.product_owner || 0,
        };

        try {
            await postProductionSeries(formattedData).unwrap();

...
const PlanningCellPage = () => {
  ...
  if (isErrorKernel) {
    return (
      <NoData
        ...
        description="نمی‌توان داده‌های مورد نیاز مانند نوع ورودی یا شناسه ورودی را بارگذاری کرد. لطفاً بعداً دوباره امتحان کنید."
  ...
  if (!planningCells?.data || planningCells.data.length === 0) {
    return (
      <div className="p-6 bg-white rounded-xl shadow-sm min-h-screen">
        <PageHeader
          title="سلول برنامه‌ریزی"
          breadcrumbItems={breadcrumbItems}
          onCreate={() => OnCreate(1)}
          createLabel="افزودن سلول برنامه‌ریزی"
        />
        ...
        <NoData
          title="هیچ سلول برنامه‌ریزی یافت نشد"
  ...
  const isKernelDataEmpty =
    !kernelData.import_types.length || !kernelData.import_ids.length;
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-sm min-h-screen">
      ...
      <DataTable
        ...
        useGetPlanningCellDetailsQuery={useGetPlanningCellDetailsQuery}

...
} from "../api/productOwnerApi.ts";
import { handleApiError } from "@/modules/shared/lib/handleApiError";
import type { CreateproductOwnerDto } from "../model/productOwner.ts";
...
export const useproductOwnerActions = () => {
  const navigate = useNavigate();
  const [postproductOwner] = usePostproductOwnersMutation();
  const [patchproductOwner] = usePatchproductOwnersMutation();
  const [deleteproductOwner] = useDeleteproductOwnersMutation();

  const deleteHandler = async (id: string) => {
    try {
      await deleteproductOwner({ id }).unwrap();
  ...
  const handleCreateConfirm = async (data: Record<string, any>) => {
    const formattedData: Partial<CreateproductOwnerDto> = {
      contact: data.contact,
    };
    try {
      await postproductOwner(formattedData).unwrap();

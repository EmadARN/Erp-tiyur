...
} from "../api/DriverApi.ts";
import { handleApiError } from "@/modules/shared/lib/handleApiError";
import type { CreatedriverDto } from "../model/driver.ts";
...
export const usedriverActions = () => {
  ...
  const [postdriver] = usePostdriversMutation();
  const [patchdriver] = usePatchdriversMutation();
  ...
  const handleCreateConfirm = async (data: Record<string, any>) => {
    ...
    const formattedData: Partial<CreatedriverDto> = {
      contact: data.contact,
      slug: data.slug,
    };
    try {
      await postdriver(formattedData).unwrap();

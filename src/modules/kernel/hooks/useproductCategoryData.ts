...
import { useGetproductCategorysQuery } from "../api/productCategoryApi.ts";
import type {
  productCategory,
  FiltersRecord,
  productCategorysResponse,
} from "../model/productCategory.ts";
import { tableHead } from "../model/productCategoryIndex.ts";

function getValueByPath(obj: productCategory, path: string): unknown {
  if (!obj || !path) return "";
...
export const useproductCategoryData = () => {
  ...
  const { data: productCategorys, isLoading } = useGetproductCategorysQuery(paramsFilterData, {
    refetchOnMountOrArgChange: true,
  });
  const [displayData, setDisplayData] = useState<productCategorysResponse | null>(null);

  useEffect(() => {
    if (productCategorys) {
      setDisplayData(productCategorys);
    }
  }, [productCategorys]);
  ...
  const handleSearch = (searchTerm: string) => {
    if (searchTerm === "" && productCategorys) {
      setDisplayData(productCategorys);
      return;
    }
    const lowerSearch = searchTerm.toLowerCase();
    if (productCategorys) {
      const result = productCategorys.filter((row) =>

...
} from "../api/productCategoryApi.ts";
import { handleApiError } from "@/modules/shared/lib/handleApiError";
import type { CreateproductCategoryDto } from "../model/productCategory.ts";
...
export const useproductCategoryActions = () => {
  ...
  const [postproductCategory] = usePostproductCategorysMutation();
  const [patchproductCategory] = usePatchproductCategorysMutation();
  const [deleteproductCategory] = useDeleteproductCategorysMutation();
  ...
  const handleCreateConfirm = async (data: Record<string, any>) => {
    const formattedData: Partial<CreateproductCategoryDto> = {
      name: data.name,
    };
    try {
      await postproductCategory(formattedData).unwrap();

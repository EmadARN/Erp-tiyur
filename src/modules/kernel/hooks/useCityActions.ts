...
import type { CreatecityDto } from "../model/city.ts";
...
export const usecityActions = () => {
  ...
  const [postcity] = usePostcitysMutation();
  const [patchcity] = usePatchcitysMutation();
  const [deletecity] = useDeletecitysMutation();
  ...
  const handleCreateConfirm = async (data: Record<string, any>) => {
    const formattedData: Partial<CreatecityDto> = {
      name: data.name,
      car_code: data.car_code,
    };
    try {
      await postcity(formattedData).unwrap();

...
import type { CreateunitDto } from "../model/unit.ts";
...
export const useunitActions = () => {
  ...
  const [postunit] = usePostunitsMutation();
  const [patchunit] = usePatchunitsMutation();
  ...
  const handleCreateConfirm = async (data: Record<string, any>) => {
    const formattedData: Partial<CreateunitDto> = {
      name: data.name,
      // city: data.city
    };
    try {
      await postunit(formattedData).unwrap();

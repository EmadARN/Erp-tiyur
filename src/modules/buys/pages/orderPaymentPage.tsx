...
const OrderPaymentPage = () => {
    ...
    const { data: invoices, isLoading: invoiceIsLoading } = useGetOrderInvoicesQuery(paramsFilterData, {
        refetchOnMountOrArgChange: true,
    });

    const { payments, isLoading, filterData, setFilterData, handleFilterOnChange, handleSearch } = useOrderPaymentData();
    const { deleteHandler, bulkDeleteHandler, handleCreateConfirm, handleUpdateConfirm } = useOrderPaymentActions();
    ...
    if (!payments?.data || payments.data.length === 0) {
        return (
            <div className="p-6 bg-white rounded-xl shadow-sm min-h-screen">
                <PageHeader title="Order Payments" breadcrumbItems={breadcrumbItems} onCreate={() => setCreateIndex(1)} createLabel="Add Order Payment" />
                ...
                <NoData title="No order payments found" description="Try adjusting filters or create a new order payment." icon={<FiCreditCard className="w-12 h-12 text-blue-500" />} />
                <CreateDialog
